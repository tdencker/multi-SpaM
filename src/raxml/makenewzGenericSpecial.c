...
#ifdef _HET
void execCore(tree *tr, volatile double *_dlnLdlz, volatile double *_d2lnLdlz2, fakeboolean isTipBranch)
#else
...
#ifdef _HET
static void topLevelMakenewz(tree *tr, double *z0, int _maxiter, double *result, fakeboolean isTipBranch)
#else
...
static void sumClassify(tree *tr, int tipCase, double *_x1, double *_x2, unsigned char *_tipX1, unsigned char *_tipX2, fakeboolean *executeModel)
{
  int 
    model = 0,
    columnCounter = 0, 
...
static void coreClassify(tree *tr, volatile double *_dlnLdlz, volatile double *_d2lnLdlz2, double *coreLZ, fakeboolean *executeModel)
{
  int 
    model, 
    branchIndex,
...
void makenewzClassify(tree *tr, int _maxiter, double *result, double *z0, double *x1_start, double *x2_start, unsigned char *tipX1,  
		      unsigned char *tipX2, int tipCase, fakeboolean *partitionConverged, int insertion)
{
  double   
    z[NUM_BRANCHES], 
    zprev[NUM_BRANCHES], 
  ...
  fakeboolean firstIteration = TRUE;
  fakeboolean outerConverged[NUM_BRANCHES];
  fakeboolean loopConverged,
...
void makenewzGeneric(tree *tr, nodeptr p, nodeptr q, double *z0, int maxiter, double *result, fakeboolean mask)
{
  int 
    i; 
