...
namespace mspamoptions
{
	...
	void printHelp()
	{
		...
		std::cout <<
		  ...
		  << "-i <string>         [Required] Input file (multi-fasta file) (default: None)" << std::endl
		  << "-o <string>         Output file (tree file in newick-format) (default:" << std::endl
		  << "                      outtree)" << std::endl
		  << "-k <int>, -w <int>  weight of the pattern (the number of matching positions)" << std::endl
		  << "                      (default: 10)" << std::endl
		  << "-d <int>            number of don't care positions in the pattern (default:" << std::endl
		  << "                      100)" << std::endl
		  << "-n <int>            number of sampled quartet blocks (default: 1000000)" << std::endl
		  << "-t <int>            number of threads (default: 1)" << std::endl
		  << "--mem-save          memory saving mode (default: False)" << std::endl
		  << "--show-stats        additional stats (mostly for debugging) (default: False)" << std::endl
	...
	void parseParameters(int argc, char *argv[])
	{
		...
		int option_index = 0;
		int c = getopt_long(argc, argv, "vai:ho:t:w:t:d:m:p:n:s:", long_options,
							&option_index);
		...
		switch (c) {
			...
			case 'p': 
				...
				if(mspamoptions::num_patterns<1) 
					printError("Number of patterns (-p) must be an integer larger than 0");
				break;
			case 'd': 
				mspamoptions::dontcare = atoi (optarg); 
				if(!isdigit(optarg[0]) || mspamoptions::dontcare<0) 
					printError("Number of don't care positions (-d) must be a positive integer");
				break;
			case 'w': 
				...
				{
					std::cout << "Weight is too large for a 32 bit integer. Setting weight to 16." << std::endl; 
    ...
    void printParameters()
    {
        ...
        << std::setw(par_width) << std::right << dontcare << std::endl
        << std::setw(text_width) << std::left << "Number of patterns: " 
        << std::setw(par_width) << std::right << num_patterns << std::endl
