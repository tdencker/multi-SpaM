...
int64_t ungappedExtension_old(std::vector<PseudoAlignment> & pa_vec, std::vector<Sequence> & sequences)
{
	int64_t nbr_removed = 0;
    //#pragma omp parallel for schedule(dynamic)
    for(auto it = pa_vec.begin(); it < pa_vec.end(); ++it)
	{
	    if(omp_get_thread_num() == 0)
	    {
	        std::cout << "\rProgress: " << (std::distance(pa_vec.begin(), it) + 1) << " / " << pa_vec.size() << std::flush;
	    }
		int xdrop = 5000; // TODO: options
		it->ungappedExtension(xdrop, sequences, 10);
	}


//	std::sort(pa_vec.begin(), pa_vec.end(), [](const PseudoAlignment & pa1, const PseudoAlignment & pa2) { return pa1.getSequenceKey() < pa2.getSequenceKey() ||  (pa1.getSequenceKey() == pa2.getSequenceKey() && pa1.getLength() < pa2.getLength()); } );
//	for(auto it = pa_vec.begin(); it < pa_vec.end();) 
//    { 
//        int64_t cur_hash = it->getDiagonalHash(sequences);
//        if(it->getLength() == last_length && cur_hash == last_hash)
//        {
//            pa_vec.erase(it);
//            nbr_removed++;
//        }
...
//            ++it;
//        }
//        last_length = it->getLength();
//        last_hash = cur_hash;
//    }
	return nbr_removed;
}

int64_t ungappedExtension(std::vector<PseudoAlignment> & pa_vec, std::vector<Sequence> & sequences)
{
    std::sort(pa_vec.begin(), pa_vec.end(), 
        [&](const PseudoAlignment & a, const PseudoAlignment & b) 
        { 
            int64_t hash_a = a.getDiagonalHash(sequences), hash_b = b.getDiagonalHash(sequences);
            return hash_a < hash_b || (hash_a == hash_b && a.getPos(0) < b.getPos(0)); 
        } 
    );  
    
    uint64_t last_hash = 0, min = 0, max = 0;
    int comp = 0, del = 0;
    for(auto cur_it = pa_vec.begin(), store_it = cur_it; ;)
    {
        ++comp;
        uint64_t pos = std::distance(sequences[cur_it->getSeq(0)].content.begin(), cur_it->getPos(0));
        if( (uint64_t)cur_it->getDiagonalHash(sequences) != last_hash || pos <= min || pos > max )
        {
            cur_it->ungappedExtension(5000, sequences, 0);
            last_hash = cur_it->getDiagonalHash(sequences);
            pos = std::distance(sequences[cur_it->getSeq(0)].content.begin(), cur_it->getPos(0));
            min = pos;
            max = pos + cur_it->getLength();
            *store_it = *cur_it;
            ++store_it;
        }
        else
        ...
        std::cout << "\rProgress: " << comp << " Deleted duplicates: " << del << std::flush;
        if(++cur_it == pa_vec.end())
        {
            int64_t res = std::distance(store_it, pa_vec.end());
            pa_vec.erase(store_it, pa_vec.end());
            std::cout << std::endl;
            return res;
        }
    }
