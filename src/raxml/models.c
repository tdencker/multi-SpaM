...
static void genericBaseFrequencies(tree *tr, const int numFreqs, rawdata *rdta, cruncheddata *cdta, int lower, int upper, int model, fakeboolean smoothFrequencies,
...
/*		  printBothOpen("Since this is probably not what you want to do, RAxML will soon exit.\n\n");*/
	  ...
	  if(zeroFreq)
	  {
	    printf("Empirical base frequencies are zero or smth.");
	    exit(-1);
	  }
...
void makeGammaCats(int rateHetModel, double alpha, double *gammaRates, int K, fakeboolean useMedian, double propInvariant)
{
  int 
    i;
...
static void setRates(double *r, int rates, fakeboolean JC69)
{
  int 
    i;
