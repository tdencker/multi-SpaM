 ...
 *
 * @param H				Length of a homolgue random region in a Dataset.
 */
...
/**
 * Long constructor, where the complete pattern and sensitivity options can be set.
 *
 ...
 * @param seq_leng		Length of sequences in database, used for variance.
 */
sensitivity::sensitivity(uint32_t size, uint32_t weight, uint32_t min_dontcare, uint32_t max_dontcare, double p, double q, double H, uint32_t seq_leng){
	this->p = p;
	this->q = q;
	this->H = H;
	this->size = size;
 ...
 *
 * @param sens 			Boolean which (de)activates the sensitivity calculation.
 *
 ...
 */
void sensitivity::Init(bool sens, bool oc, bool improve, bool randpatleng, bool quiet, bool silent, const char *outFile){
	if(!init){
		this->sens = sens;
		this->oc = oc;
...
/*---Improvement-------------------------------------------------------------*/
/**
 * Improvement Method, which starts the improvement method from the variance object.
 * 
 * @param limits 		Number of tries of random permutation to optimize the ov/variance of the patternset
 *
 * @param opt_oc		Number of reinitializations of the patternset, to find the best oc/variance of the permutated patternsets.
 */
...
/**
 * Improvement Method, which improves the sensitivity improvement incl. the variance/oc improvement.
 * 
 * @param limits 		Number of tries of random permutation to optimize the ov/variance of the patternset
 *
 * @param opt_oc		Number of reinitializations of the patternset, to find the best oc/variance of the permutated patternsets.
 *
 * @param opt_sens		Number of reinitializations of the patternset, to find the best sensitivity of the best oc/variance optimized permutated patternsets. 
 */
		...
		for(uint32_t i = 1; i <= opt_sens; i++){
			...
			if(sens_value > best_sens_value){
				...
				if(!silent && opt_sens != 1){
					if(!quiet){
						std::cout << "\n*** BETTER PATTERN " << better_pattern << " *** \t(sensitivity optimization)" << std::endl;
 ...
 *
 * @return 				The sensitivity of the current set (0 if not calculated)
 */
 ...
 *
 * @return 				Current length of a random homolgue region in a possible datasat.
 */
