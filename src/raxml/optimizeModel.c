...
static fakeboolean optimizeRatesBFGS(tree *tr);
...
void scaleBranches(tree *tr, fakeboolean fromFile);
...
#ifdef _HET
static void setRateModel(tree *tr, int model, double rate, int position, fakeboolean isHet)
#else
static void setRateModel(tree *tr, int model, double rate, int position)
#endif
{
...
/*    assert(rate >= RATE_MIN && rate <= RATE_MAX);*/
    ...
    {
        //printf("bad rate: %f RATEMIN: %f RATE_MAX: %f\n", rate, RATE_MIN, RATE_MAX);
    }
...
static void evaluateChange(tree *tr, int rateNumber, double *value, double *result, fakeboolean* converged, int whichFunction, int numberOfModels, linkageList *ll)
{ 
  int 
    i, 
    k, 
			 ...
			 int whichFunction, int rateNumber, tree *tr, linkageList *ll, double *lim_inf, double *lim_sup)
{
  ...
  fakeboolean *converged = (fakeboolean *)rax_malloc(sizeof(fakeboolean) * numberOfModels);
  fakeboolean allConverged;
...
static fakeboolean AAisGTR(tree *tr)
{
  int 
    i, 
    count = 0;
...
static fakeboolean AAisUnlinkedGTR(tree *tr)
{
  int 
    i, 
    count = 0;
...
static int optAlphasGeneric(tree *tr, double modelEpsilon, linkageList *ll)
{
  int 
    i,
...
/*        assert((tr->partitionData[index].alpha < alphaMaybeNoRateHet));*/
...
void updatePerSiteRates(tree *tr, fakeboolean scaleRates)
{
  int 
    i,
    model;
...
static void optModel(tree *tr, int numProteinModels, int *bestIndex, double *bestScores, fakeboolean empiricalFreqs)
{
  int
    i,
    model;
...
static void autoProtein(tree *tr)
{
  ...
  if(countAutos > 0)
	...
	*bestIndexEmpFreqs = (int*)rax_malloc(sizeof(int) * tr->NumberOfModels);
      ...
      fakeboolean
	*oldFreqs =  (fakeboolean*)rax_malloc(sizeof(fakeboolean) * tr->NumberOfModels);
...
int modOpt(tree *tr, analdef *adef, fakeboolean resetModel, double likelihoodEpsilon)
{ 
...
/*	optScaler(tr, modelEpsilon, scalerList); */
	...
	{	  
	case GAMMA_I:
	  if(optAlphasGeneric(tr, modelEpsilon, alphaList) != 0)
	  {
        rax_free(unlinked);
        rax_free(linked);
...
static void dfpmin(double *p, int n, double *lower, double *upper, double gtol, int *iter, double *fret, tree *tr,  fakeboolean *bfgsConverged);
...
static double minimizeMultiDimen(double *guess, int ndim, double *lower, double *upper, fakeboolean *bound_check, double gtol, tree *tr, fakeboolean *bfgsConverged) 
{
  int 
    i, 
    iter,
...
static fakeboolean optimizeRatesBFGS(tree *tr)
{
  ...
  fakeboolean
    bfgsConverged = TRUE,
    *bound_check_GTR = (fakeboolean*)rax_malloc(sizeof(fakeboolean) * (nGTR + 1));
